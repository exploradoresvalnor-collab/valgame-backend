...
describe('🔒 TEST DE SEGURIDAD Y PROBABILIDADES', () => {
    ...
    afterAll(async () => {
        ...
        await User_1.User.deleteMany({ email: new RegExp(`security_test_${baseTimestamp}`) });
        await User_1.User.deleteMany({ email: new RegExp(`prob_test_${baseTimestamp}`) });
    ...
    describe('🔐 PARTE 1: Validación de Seguridad', () => {
        ...
        it('1.3 - Debe rechazar tokens JWT expirados', async () => {
            ...
            const expiredToken = jsonwebtoken_1.default.sign({ id: 'fake_user_id' }, process.env.JWT_SECRET || 'secret', { expiresIn: '0s' });
        ...
        it('1.6 - Debe generar token JWT válido en login', async () => {
            ...
            await (0, supertest_1.default)(app_1.default).post('/auth/register').send({ email, username: `user_${uniqueId}`, password });
            const user = await User_1.User.findOne({ email });
            if (user) {
                await (0, supertest_1.default)(app_1.default).get(`/auth/verify/${user.verificationToken}`);
            }
            const res = await (0, supertest_1.default)(app_1.default)
                .post('/auth/login')
                .send({ email, password })
            ...
            const decoded = jsonwebtoken_1.default.verify(validToken, process.env.JWT_SECRET || 'secret');
    ...
    describe('🎲 PARTE 2: Validación de Probabilidades de Paquetes', () => {
        let packageId;
        let userForProbability;
        let probabilityToken;
        beforeAll(async () => {
            ...
            await UserPackage_1.default.insertMany(userPackages);
        ...
        it('2.1 - Debe abrir 20 paquetes y validar distribución de rangos', async () => {
            ...
            for (let i = 0; i < 20; i++) {
                ...
                const updatedUser = await User_1.User.findById(userForProbability._id);
        ...
        it('2.2 - Debe agregar personajes correctamente por cada apertura', async () => {
            ...
            const userBefore = await User_1.User.findById(userForProbability._id);
            ...
            await (0, supertest_1.default)(app_1.default)
                ...
                .expect(200);
            const userAfter = await User_1.User.findById(userForProbability._id);
