...
const LevelRequirement_1 = __importDefault(require("../../src/models/LevelRequirement"));
let mongod;
let token;
let userId;
let dungeonId;
...
describe('Dungeon Reward Flow', () => {
    ...
    beforeAll(async () => {
        ...
        [weapon, armor, potion, rewardItem] = await Promise.all([
            Item_1.Item.create(TEST_WEAPON),
            Item_1.Item.create(TEST_ARMOR),
            Item_1.Item.create(TEST_POTION),
            Item_1.Item.create({
                ...TEST_WEAPON,
        ...
        const dungeon = await Dungeon_1.default.create(dungeonData);
        ...
        await GameSetting_1.default.create({
        ...
        await LevelRequirement_1.default.insertMany(levelDocs);
    ...
    it('should complete full dungeon flow: register → get character → complete dungeon → receive rewards → list item', async () => {
        ...
        const createdUser = await User_1.User.findOne({ email: TEST_USER.email });
        if (createdUser) {
        ...
        if (createdUser) {
            ...
            createdUser.inventarioConsumibles.push({ consumableId: potion._id, usos_restantes: 1 });
            await createdUser.save();
        ...
        const dbUser = await User_1.User.findById(userId);
        if (dbUser) {
            ...
            if (dbUser.inventarioConsumibles.length === 0) {
                dbUser.inventarioConsumibles.push({ consumableId: potion._id, usos_restantes: 1 });
            }
            await dbUser.save();
        ...
        const dbUserBeforeEquip = await User_1.User.findById(userId);
        if (!dbUserBeforeEquip)
            throw new Error('Usuario no encontrado en DB antes de equipar');
        const personaje = dbUserBeforeEquip.personajes.find((p) => p.personajeId === characterId);
        ...
        dbUserBeforeEquip.markModified && dbUserBeforeEquip.markModified('personajes');
        ...
        const useConsumableRes = await (0, supertest_1.default)(app_1.default)
        ...
        const updatedUserRes = await (0, supertest_1.default)(app_1.default)
            .get('/api/users/me')
            .set('Authorization', `Bearer ${token}`);
        ...
        const updatedPersonaje = updatedUserRes.body.personajes.find((p) => p.personajeId === characterId);
        ...
        // 7. Iniciar mazmorra
        const dungeonStartResponse = await (0, supertest_1.default)(app_1.default)
        ...
        const updatedUserResponse = await (0, supertest_1.default)(app_1.default)
            .get('/api/users/me')
            .set('Authorization', `Bearer ${token}`);
        ...
        const rewardIdStr = rewardItem._id.toString();
        ...
        const listingResponse = await (0, supertest_1.default)(app_1.default)
        ...
        expect(searchResponse.body.listings.find((l) => String(l.itemId) === String(itemId))).toBeDefined();
    });
    it('should handle dungeon defeat correctly and mark character as wounded', async () => {
        // Crear un dungeon más difícil
        const hardDungeon = await Dungeon_1.default.create({
            ...TEST_DUNGEON,
        ...
        await (0, supertest_1.default)(app_1.default).post('/auth/register').send(TEST_USER);
        // Marcar usuario como verificado en DB para permitir login en tests
        const maybeUser = await User_1.User.findOne({ email: TEST_USER.email });
        if (maybeUser) {
        ...
        const dbUser2 = await User_1.User.findById(loginRes.body.user?.id || loginRes.body.userId);
        ...
        const dungeonRes = await (0, supertest_1.default)(app_1.default)
        ...
        const updatedUserRes = await (0, supertest_1.default)(app_1.default)
            .get('/api/users/me')
            .set('Authorization', `Bearer ${token}`);
        const character = updatedUserRes.body.personajes.find((p) => p.personajeId === characterId);
