...
const listItemInMarketplace = async (req, res) => {
    try {
        ...
        const isEquipment = user.inventarioEquipamiento && user.inventarioEquipamiento.some((id) => id.toString() === itemIdObj.toString());
        // Check consumables (format: [{consumableId: ObjectId, usos_restantes: Number}])
        const isConsumable = user.inventarioConsumibles && user.inventarioConsumibles.some((c) => c.consumableId?.toString() === itemIdObj.toString() && c.usos_restantes > 0);
