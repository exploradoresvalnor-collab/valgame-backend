...
const buyValPackage = async (req, res) => {
    ...
    if (!(0, mongoose_1.isValidObjectId)(packageId)) {
        return res.status(400).json({ error: 'ID de paquete inválido.' });
    }
    try {
        // Lógica: Iniciar checkout y crear Purchase en estado 'pending'
        const pkg = await Package_1.default.findOne({ _id: packageId });
        if (!pkg) {
            return res.status(404).json({ error: 'Paquete no encontrado.' });
        }
        // Crear orden de checkout con el proveedor (mock en MVP)
        const checkout = await payment_service_1.default.createCheckout(userId, packageId, pkg.precio_usdt);
