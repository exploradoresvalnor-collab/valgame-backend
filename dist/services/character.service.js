...
exports.handleLevelUp = void 0;
exports.calcularXpPorRango = calcularXpPorRango;
/**
 ...
 */
const handleLevelUp = (character, levelRequirements, gameSettings) => {
    let leveledUp = false;
    const levelUpLog = [];
    ...
    while (character.nivel < maxLevel) {
        ...
        try {
            const map = gameSettings.exp_req_multiplier_por_rango;
            if (map) {
                if (typeof map === 'object' && map[character.rango] != null) {
                    ...
                    if (val != null)
                        expReqMultiplier = Number(val) || 1;
                }
            }
        }
        catch (err) {
            console.warn('Advertencia al leer exp_req_multiplier_por_rango:', err?.message || err);
        }
        ...
        if (character.progreso >= expNeeded) {
            ...
            try {
                const map = gameSettings.aumento_stats_por_nivel;
                ...
                console.warn('Advertencia al leer aumento_stats_por_nivel:', err?.message || err);
            }
...
// ------------------- Helpers exportados -------------------
function getMultiplier(map, rango, fallback = 1) {
    if (!map)
        return fallback;
    try {
...
function calcularXpPorRango(nivel, rango, levelReqObj, expBase, settings) {
