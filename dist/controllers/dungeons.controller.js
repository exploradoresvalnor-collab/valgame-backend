...
const PlayerStat_1 = __importDefault(require("../models/PlayerStat")); // Importa el modelo PlayerStat
...
const startDungeon = async (req, res) => {
    ...
    try {
        ...
        const [user, dungeon, gameSettings, levelRequirements] = await Promise.all([
            User_1.User.findById(userId).populate({ path: 'personajes.equipamiento', model: 'Item' }),
            Dungeon_1.default.findById(dungeonId),
            GameSetting_1.default.findOne(),
            LevelRequirement_1.default.find() // Cargar todos los requisitos de nivel
        ]);
        if (!user)
        ...
        if (!levelRequirements || levelRequirements.length === 0)
            return res.status(500).json({ error: 'Requisitos de nivel no encontrados.' });
        ...
        const statsEscaladas = (0, dungeonProgression_1.calcularStatsEscaladas)(dungeon.stats, dungeonProgress.nivel_actual, { multiplicador_stats_por_nivel: dungeon.nivel_sistema?.multiplicador_stats_por_nivel || 0.15 });
        ...
        while (battleResult === 'en_curso') {
            if (playerTurn) {
                ...
                if (combatTeam.every(c => c.saludActual <= 0))
        ...
        let puntosGanados = 0;
        let progresoMazmorraResult = null;
        if (battleResult === 'victoria') {
            combatLog.push('¡VICTORIA! Has superado la mazmorra.');
            // --- Calcular recompensas escaladas ---
            const recompensasEscaladas = (0, dungeonProgression_1.calcularRecompensasEscaladas)({ expBase: dungeon.recompensas.expBase, valBase: dungeon.recompensas.valBase || 0 }, dungeonProgress.nivel_actual, {
            ...
            const multiplicadorDrop = (0, dungeonProgression_1.calcularMultiplicadorDrop)(dungeonProgress.nivel_actual, dungeon.nivel_sistema?.multiplicador_drop_por_nivel || 0.05);
            ...
            const possibleItemsInfo = await Item_1.Item.find({ '_id': { $in: possibleDropIds } });
            const itemsInfoMap = new Map(possibleItemsInfo.map(item => [String(item._id), item]));
            for (const drop of dropTableCompleto) {
                ...
                if (Math.random() < probabilidadFinal) {
                    ...
                    const genericItemInfo = itemInfo;
            ...
            const tiempoEstimado = (0, dungeonProgression_1.calcularTiempoEstimado)(statsEscaladas.vida, statsEscaladas.ataque);
            puntosGanados = (0, dungeonProgression_1.calcularPuntosVictoria)(combatDuration, tiempoEstimado, saludRestantePorcentaje, user.dungeon_streak || 0);
            progresoMazmorraResult = (0, dungeonProgression_1.procesarVictoria)(dungeonProgress, puntosGanados, combatDuration);
            ...
            await PlayerStat_1.default.create({
                userId: user._id,
            ...
            await Ranking_1.Ranking.findOneAndUpdate({ userId: user._id, periodo: 'global' }, {
                $inc: {
