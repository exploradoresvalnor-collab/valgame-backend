...
router.post('/enter/:dungeonId', auth, (req: Request, res: Response, next) => {
  ...
  return (startDungeon as any)(req, res, next as any);
});

// GET /api/dungeons/:dungeonId/progress - Obtener progreso del usuario en una mazmorra específica
router.get('/:dungeonId/progress', auth, async (req, res) => {
  try {
    const { dungeonId } = req.params;
    const userId = (req as any).userId;
...
router.get('/:dungeonId/session/:sessionId', auth, async (req: Request, res: Response) => {
  // Delegar a la ruta de progreso actual. sessionId se ignora en esta versión.
  (req as any).params = { ...req.params, dungeonId: req.params.dungeonId };
